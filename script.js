...
// Hausaufgaben und Material Modul rendern - GEÄNDERT für neue Zählerstruktur
function renderHomeworkModule() {
    ...
    studentsToRender.forEach((student, index) => {
        ...
        // Die Zähler werden rechtsbündig angezeigt, gefolgt von den Buttons
        hwCounters.innerHTML = `
            <div class="hw-counter-buttons" style="margin-left: auto;">
...
// NEU: Erhöhung eines Zählers bestätigen
...
// NEU: Verringerung eines Zählers bestätigen
...
// Endnote berechnen - GEÄNDERT: Verwendet jetzt präzise Kommanoten statt ganzer Noten
function calculateFinalGrade(projects, oralGrade, oralWeight) {
    ...
    if (!oralGrade || oralGrade === '') {
        ...
        // Präzise Kommanote für mündliche Note verwenden
...
function renderOverviewModule() {
    ...
    studentsToRender.forEach((student, studentIndex) => {
        ...
        // Projekte - Hier jetzt mit Kommanoten
        ...
        if (projectGrades.length > 0) {
            ...
            tr.innerHTML += `<td><span class="badge ${projectGradeClass}">${roundGrade(projectAverage)}</span> (${projectAverage.toFixed(2)})</td>`;
        } else {
            tr.innerHTML += `<td><span class="badge badge-secondary">Keine</span></td>`;
        }
        ...
        tr.innerHTML += `
            <td>
                <button class="btn btn-sm btn-primary btn-square" onclick="showIndividualGradeModal(${studentIndex})">
                    <i class="fas fa-edit"></i>
                </button>
            </td>
        `;
...
function toggleAutoBackup(enabled) {
    ...
    if (enabled) {
        ...
        scheduleNextAutoBackup();
        swal("Info", "Automatische Backups wurden aktiviert.", "info");
    } else {
        ...
        swal("Info", "Automatische Backups wurden deaktiviert.", "info");
    }
    
    updateBackupInfo();
}

// Automatische Backup-Einstellungen aktualisieren
function updateAutoBackupSettings() {
    const maxBackupsSelect = safeGetElement('max-auto-backups');
    if (maxBackupsSelect) {
        maxAutoBackups = parseInt(maxBackupsSelect.value) || 5;
        saveData();
        
        // Überprüfen, ob wir zu viele Auto-Backups haben
        cleanupAutoBackups();
...
// Nächstes automatisches Backup berechnen
function getNextAutoBackupDate() {
    // Letztes automatisches Backup finden
    const autoBackups = backups.filter(backup => backup.type === 'auto');
    
    if (autoBackups.length === 0) {
        // Wenn kein Auto-Backup vorhanden ist, sofort erstellen
        return null;
    }
    
    // Neuestes Backup finden
    const latestBackup = autoBackups.sort((a, b) => new Date(b.date) - new Date(a.date))[0];
    const lastBackupDate = new Date(latestBackup.date);
    
    // Nächstes Backup in 24 Stunden nach dem letzten
    const nextBackupDate = new Date(lastBackupDate);
    nextBackupDate.setHours(nextBackupDate.getHours() + 24);
...
function scheduleNextAutoBackup() {
    // Falls bereits ein Timer läuft, zurücksetzen
    if (autoBackupInterval) {
        clearTimeout(autoBackupInterval);
        autoBackupInterval = null;
    }
    
    // Nächstes Backup-Datum berechnen
    const nextBackupDate = getNextAutoBackupDate();
    
    if (!nextBackupDate) {
        // Sofort ein Backup erstellen, wenn keines vorhanden ist
        createAutoBackup();
        return;
    }
    
    // Zeit bis zum nächsten Backup berechnen
    const now = new Date();
    let timeUntilNextBackup = nextBackupDate.getTime() - now.getTime();
    
    // Falls die Zeit negativ ist (Backup überfällig), sofort erstellen
    if (timeUntilNextBackup <= 0) {
        createAutoBackup();
        return;
    }
    
    // Timer für das nächste Backup setzen
...
function createAutoBackup() {
    // Backup erstellen
    createBackupInternal('auto');
    
    // Alte Auto-Backups bereinigen
    cleanupAutoBackups();
...
// Alte automatische Backups bereinigen
function cleanupAutoBackups() {
    // Nur automatische Backups filtern
    const autoBackups = backups.filter(backup => backup.type === 'auto');
    
    // Wenn die Anzahl der Auto-Backups unter dem Limit liegt, nichts tun
    if (autoBackups.length <= maxAutoBackups) {
        return;
    }
    
    // Auto-Backups nach Datum sortieren (älteste zuerst)
    const sortedAutoBackups = autoBackups.sort((a, b) => new Date(a.date) - new Date(b.date));
    
    // Überzählige Backups löschen
    const backupsToDelete = sortedAutoBackups.length - maxAutoBackups;
    
    for (let i = 0; i < backupsToDelete; i++) {
        const backupToDelete = sortedAutoBackups[i];
        const backupIndex = backups.findIndex(b => b.date === backupToDelete.date);
...
function createBackup(event) {
    // Stoppt die Ereignispropagierung, falls es von einem Button aufgerufen wurde
    if (event && event.stopPropagation) {
        event.stopPropagation();
    }
    
    createBackupInternal('manual');
    swal("Erfolg", "Backup wurde erstellt.", "success");
}

// Internes Backup erstellen
function createBackupInternal(type) {
    // Backup-Objekt erstellen
    const backup = {
        date: new Date().toISOString(),
        type: type, // 'manual' oder 'auto'
...
// Backup wiederherstellen - Modal anzeigen
function showRestoreBackupModal(backupIndex) {
    ...
    // Modal-Datum aktualisieren
    const restoreBackupDateElement = safeGetElement('restore-backup-date');
    if (restoreBackupDateElement) {
        restoreBackupDateElement.textContent = backupDate.toLocaleDateString('de-DE', { 
            day: '2-digit', 
            month: '2-digit', 
            year: 'numeric' 
        }) + ' ' + backupDate.toLocaleTimeString('de-DE', {
    ...
    // Backup-Index für spätere Verwendung speichern
...
function confirmRestoreBackup() {
    if (currentBackupToRestore === null || currentBackupToRestore < 0 || currentBackupToRestore >= backups.length) {
        swal("Fehler", "Das ausgewählte Backup wurde nicht gefunden.", "error");
        hideModal();
        return;
    }
    ...
    try {
        // Daten aus dem Backup laden
        const restoredClasses = JSON.parse(backup.data);
        
        // Aktuellen Zustand sichern (automatisches Backup für Wiederherstellungsfehler)
        ...
        swal("Fehler", "Beim Wiederherstellen des Backups ist ein Fehler aufgetreten.", "error");
        hideModal();
    }
}
...
function exportAllData(event) {
    ...
    try {
        ...
        const exportData = {
            classes: classes,
            backups: backups,
            version: '1.0', // Versionsnummer für zukünftige Kompatibilität
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `schulverwaltung_export_${dateStr}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        
        swal("Erfolg", "Daten wurden erfolgreich exportiert.", "success");
    } catch (error) {
        console.error("Fehler beim Exportieren:", error);
        swal("Fehler", "Beim Exportieren der Daten ist ein Fehler aufgetreten.", "error");
    }
}
...
function importBackupFile(event) {
    ...
    reader.onload = function(e) {
        ...
        try {
            ...
            // Daten aus der Datei parsen
            const importData = JSON.parse(e.target.result);
            
            // Prüfen, ob es sich um ein gültiges Exportformat handelt
            if (!importData.classes) {
                swal("Fehler", "Die ausgewählte Datei enthält keine gültigen Daten.", "error");
                return;
            }
            
            // Importierte Klassen einfügen
            classes = importData.classes;
            
            // Importierte Backups hinzufügen, falls vorhanden
            if (importData.backups) {
                backups = [...backups, ...importData.backups];
                
                // Duplikate entfernen
                backups = backups.filter((backup, index, self) => 
                    index === self.findIndex(b => b.date === backup.date)
                );
            }
            ...
            swal("Fehler", "Beim Importieren der Daten ist ein Fehler aufgetreten.", "error");
        }
...
// Funktion zum Ein- und Ausklappen der Backup-Verwaltung
...
// Helper-Funktionen für die Backup-Buttons in der Header-Leiste
function handleImportButtonClick(event) {
    // Stoppt die Ereignispropagierung, damit toggleBackupPanel nicht ausgelöst wird
    if (event) {
        event.stopPropagation();
    }
