            ...
            <div id="notenstudentDetails-${studentIndex}" class="student-details ${student.notenExpanded ? 'show' : ''}">
                <div class="form-row-with-notes">
                    ...
                    <div>
                        ...
                        <div class="sonstiges-list">
        `;
        
        // Abschreibtext und Nachsitzen Einträge sammeln
        const sonstigesEntries = [];
        if (student.hwHistory) {
            student.hwHistory.forEach(entry => {
                if (entry.type === 'abschreibtext' || entry.type === 'nachsitzen') {
                    const entryDate = new Date(entry.date);
                    const formattedDate = entryDate.toLocaleDateString('de-DE');
                    const typeText = entry.type === 'abschreibtext' ? 'Abschreibtext' : 'Nachsitzen';
                    sonstigesEntries.push(`${formattedDate}: ${typeText}`);
                }
            });
        }
        
        if (sonstigesEntries.length === 0) {
            studentDetails += `<span class="text-muted">Keine Einträge</span>`;
        } else {
            studentDetails += `<ul>`;
            sonstigesEntries.forEach(entry => {
                studentDetails += `<li>${entry}</li>`;
            });
            studentDetails += `</ul>`;
        }
...
function renderDesk(desk) {
    ...
    deskElement.addEventListener('click', (e) => {
        ...
        if (classes[activeClassId] && classes[activeClassId].sitzplan && classes[activeClassId].sitzplan.currentMode === 'oral') {
            ...
            if (desk.studentIndex !== null) {
                if (deskClickHistory[desk.id].lastClickType && deskClickHistory[desk.id].lastClickType !== type && now - deskClickHistory[desk.id].lastClickTime < 10000) {
                    // Korrektur: Verringere den falschen, erhöhe den richtigen
                    ...
                    showVisualFeedback(deskElement, type);
