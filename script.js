...
function showModule(module) {
    // Vor dem Tab-Wechsel: Aktuelle Zeugnis-Notizen speichern, falls wir gerade im Zeugnis-Tab sind
    if (activeModule === 'zeugnis') {
        const studentCards = document.querySelectorAll('#zeugnis-container .student-card');
        studentCards.forEach((card, index) => {
            if (typeof saveStudentNotes === 'function') {
                saveStudentNotes(index);
            }
        });
    }
                ...
                <div class="zeugnis-section">
                    ...
                    <div class="zeugnis-notes-container">
                        <div class="zeugnis-notes-left">
                            <textarea class="form-control notes-textarea" id="notes-left-${index}" rows="13" placeholder="Linke Notizen..." oninput="saveStudentNotes(${index})">${leftNotes}</textarea>
                        </div>
                        <div class="zeugnis-notes-right">
                            <textarea class="form-control notes-textarea" id="notes-right-${index}" rows="13" placeholder="Rechte Notizen..." oninput="saveStudentNotes(${index})">${rightNotes}</textarea>
                        </div>
                    </div>
                </div>
                <div class="zeugnis-section summary-section">
                    <h4>Zusammenfassung und Zeugnisnote</h4>
                    <textarea class="form-control notes-textarea" id="notes-summary-${index}" rows="4" placeholder="Zusammenfassung und Zeugnisnote..." oninput="saveStudentNotes(${index})">${summaryNotes}</textarea>
                </div>
            </div>
        `;
...
function saveStudentNotes(studentIndex) {
    ...
    // Sicherstellen, dass die globale Variable 'classes' aktuell ist
    if (window.classes) {
        window.classes = classes;
    }
    ...
    // Explizites Speichern mit studentIndex für den Cloud-Sync-Optimierer
