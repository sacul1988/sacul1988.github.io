...
// Variable für das Debouncing des Speichervorgangs
let saveTimeout = null;

// Daten im localStorage speichern
function saveData() {
    // Falls ein alter Timeout läuft, diesen stoppen
    if (saveTimeout) clearTimeout(saveTimeout);

    // Verzögertes Speichern (Debouncing), um Firestore-Limits zu schonen
    // und Datenverlust beim Tippen zu minimieren
    ...
    }, 500); // 500ms warten, bevor wirklich gespeichert wird
}
...
function renderZeugnisModule() {
                ...
                <div class="zeugnis-section">
                    ...
                    <div class="zeugnis-notes-container">
                        <div class="zeugnis-notes-left">
                            <textarea class="form-control notes-textarea" id="notes-left-${index}" rows="13" placeholder="Linke Notizen..." oninput="saveStudentNotes(${index})" onblur="saveStudentNotes(${index})">${leftNotes}</textarea>
                        </div>
                        <div class="zeugnis-notes-right">
                            <textarea class="form-control notes-textarea" id="notes-right-${index}" rows="13" placeholder="Rechte Notizen..." oninput="saveStudentNotes(${index})" onblur="saveStudentNotes(${index})">${rightNotes}</textarea>
                        </div>
                    </div>
                </div>
                <div class="zeugnis-section summary-section">
                    <h4>Zusammenfassung und Zeugnisnote</h4>
                    <textarea class="form-control notes-textarea" id="notes-summary-${index}" rows="4" placeholder="Zusammenfassung und Zeugnisnote..." oninput="saveStudentNotes(${index})" onblur="saveStudentNotes(${index})">${summaryNotes}</textarea>
                </div>
            </div>
        `;
    ...
    const zeugnisListener = function(event) {
        if (event.target.matches('textarea[id^="notes-left-"], textarea[id^="notes-right-"]')) {
            if (event.key === 'Enter') {
                // Verhindere das Standard-Enter, weils wir das Format selbst bestimmen
                event.preventDefault();
                event.stopPropagation(); // Stoppe das Event hier
                ...
                // Füge den Zeilenumbruch und das Aufzählungszeichen ein
                const insertText = '\n- ';
                textarea.value = value.substring(0, start) + insertText + value.substring(end);
                
                // Cursor hinter das neue Aufzählungszeichen setzen
